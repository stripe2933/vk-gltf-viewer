...
vk_gltf_viewer::vulkan::Frame::Frame(std::shared_ptr<const Renderer> _renderer, const SharedData &sharedData)
    ...
    vku::allocateDescriptorSets(sharedData.gpu.device, *descriptorPool, {
        { sharedData.rendererDescriptorSetLayout, std::ref(rendererSet) },
        { sharedData.mousePickingDescriptorSetLayout, std::ref(mousePickingSet) },
        { sharedData.jumpFloodComputePipeline.descriptorSetLayout, { std::ref(hoveringNodeJumpFloodSet), std::ref(selectedNodeJumpFloodSet) } },
        { sharedData.outlineDescriptorSetLayout, { std::ref(hoveringNodeOutlineSet), std::ref(selectedNodeOutlineSet) } },
        { sharedData.weightedBlendedCompositionDescriptorSetLayout, std::ref(weightedBlendedCompositionSet) },
        { sharedData.inverseToneMappingDescriptorSetLayout, std::ref(inverseToneMappingSet) },
        { sharedData.bloomComputePipeline.descriptorSetLayout, std::ref(bloomSet) },
        { sharedData.bloomApplyDescriptorSetLayout, std::ref(bloomApplySet) },
    });

    // Update descriptor sets.
    sharedData.gpu.device.updateDescriptorSets(
        sharedData.rendererDescriptorSetLayout.getWriteDescriptorSet<0>(rendererSet, 0, vku::lvalue(vk::DescriptorBufferInfo {
            cameraBuffer,
            0, vk::WholeSize,
        })),
        {});
        ...
        if (gltfAsset->mousePickingInput) {
            ...
            if (rect.extent.width == 1 && rect.extent.height == 1) {
                ...
                const std::span packedBits {
                    ...
                    math::divCeil(gltfAsset->assetExtended->asset.nodes.size(), 32UZ),
                };
...
void vk_gltf_viewer::vulkan::Frame::recordCommandsAndSubmit(Swapchain &swapchain) const {
    ...
    std::optional<bool> hoveringNodeJumpFloodForward{}, selectedNodeJumpFloodForward{};
    {
        ...
        if (hoveringNode) {
            ...
            sharedData.gpu.device.updateDescriptorSets(
                sharedData.outlineDescriptorSetLayout.getWriteDescriptorSet<0>(hoveringNodeOutlineSet, 0, vku::lvalue(vk::DescriptorImageInfo {
                    {},
        ...
        if (selectedNodes) {
            ...
            sharedData.gpu.device.updateDescriptorSets(
                sharedData.outlineDescriptorSetLayout.getWriteDescriptorSet<0>(selectedNodeOutlineSet, 0, vku::lvalue(vk::DescriptorImageInfo {
                    {},
...
void vk_gltf_viewer::vulkan::Frame::setViewportExtent(const vk::Extent2D &extent) {
    ...
    sharedData.gpu.device.updateDescriptorSets({
        sharedData.weightedBlendedCompositionDescriptorSetLayout.getWriteDescriptorSet<0>(weightedBlendedCompositionSet, 0, vku::lvalue({
        ...
        sharedData.inverseToneMappingDescriptorSetLayout.getWriteDescriptorSet<0>(inverseToneMappingSet, 0, vku::lvalue(vk::DescriptorImageInfo {
            {},
            *viewport->sceneAttachmentGroup.colorImageView,
            vk::ImageLayout::eShaderReadOnlyOptimal,
        })),
        sharedData.bloomApplyDescriptorSetLayout.getWriteDescriptorSet<0>(bloomApplySet, 0, vku::lvalue(vk::DescriptorImageInfo {
            {},
            *viewport->sceneAttachmentGroup.colorImageView,
            vk::ImageLayout::eGeneral,
        })),
        sharedData.jumpFloodComputePipeline.descriptorSetLayout.getWriteDescriptorSet<0>(hoveringNodeJumpFloodSet, 0, vku::lvalue(vk::DescriptorImageInfo {
            {},
            *viewport->hoveringNodeOutlineJumpFloodResources.imageView,
            vk::ImageLayout::eGeneral,
        })),
        sharedData.jumpFloodComputePipeline.descriptorSetLayout.getWriteDescriptorSet<0>(selectedNodeJumpFloodSet, 0, vku::lvalue(vk::DescriptorImageInfo {
            {},
            *viewport->selectedNodeOutlineJumpFloodResources.imageView,
            vk::ImageLayout::eGeneral,
        })),
        sharedData.inverseToneMappingDescriptorSetLayout.getWriteDescriptorSet<1>(inverseToneMappingSet, 0, vku::lvalue(vk::DescriptorImageInfo {
            {},
            *viewport->bloomMipImageViews[0],
            vk::ImageLayout::eGeneral,
        })),
        sharedData.bloomComputePipeline.descriptorSetLayout.getWriteDescriptorSet<0>(bloomSet, 0, vku::lvalue(vk::DescriptorImageInfo {
            {},
            *viewport->bloomImageView,
            vk::ImageLayout::eGeneral,
        })),
        sharedData.bloomComputePipeline.descriptorSetLayout.getWriteDescriptorSet<1>(bloomSet, 0, vku::lvalue([this] {
        ...
        sharedData.bloomApplyDescriptorSetLayout.getWriteDescriptorSet<1>(bloomApplySet, 0, vku::lvalue(vk::DescriptorImageInfo {
            {},
            *viewport->bloomMipImageViews[0],
            vk::ImageLayout::eShaderReadOnlyOptimal,
        })),
...
void vk_gltf_viewer::vulkan::Frame::updateAsset() {
    ...
    if (std::uint32_t variableDescriptorCount = getVariableDescriptorCount(inner.assetExtended->asset);
        ...
        vku::allocateDescriptorSets(
            sharedData.gpu.device,
            *descriptorPool,
            {
                { *sharedData.assetDescriptorSetLayout, std::ref(assetDescriptorSet) },
            },
            &vku::lvalue(vk::DescriptorSetVariableDescriptorCountAllocateInfo { vk::ArrayProxyNoTemporaries<const std::uint32_t> { variableDescriptorCount } }));
        assetDescriptorSetReallocated = true;
    }

    // Update the descriptors that are unrelated to the asset textures.
    sharedData.gpu.device.updateDescriptorSets({
        sharedData.mousePickingDescriptorSetLayout.getWriteDescriptorSet<0>(mousePickingSet, 0, vku::lvalue(vk::DescriptorBufferInfo {
            inner.mousePickingResultBuffer.buffer,
            0, vk::WholeSize,
        })),
        sharedData.assetDescriptorSetLayout.getWriteDescriptorSet<0>(assetDescriptorSet, 0, vku::lvalue(vk::DescriptorBufferInfo {
            inner.assetExtended->primitiveBuffer.buffer,
            0, vk::WholeSize,
        })),
        sharedData.assetDescriptorSetLayout.getWriteDescriptorSet<1>(assetDescriptorSet, 0, vku::lvalue(vk::DescriptorBufferInfo {
            inner.nodeBuffer.buffer,
            0, vk::WholeSize,
        })),
        sharedData.assetDescriptorSetLayout.getWriteDescriptorSet<2>(assetDescriptorSet, 0, vku::lvalue(vk::DescriptorBufferInfo {
            inner.assetExtended->materialBuffer.buffer,
            0, vk::WholeSize,
        })),
    }, {});
    ...
    if (assetDescriptorSetReallocated) {
        ...
        descriptorWrites.push_back(sharedData.assetDescriptorSetLayout.getWriteDescriptorSet<4>(assetDescriptorSet, 0, fallbackImageInfo));
    }
    ...
    if (!samplerInfos.empty()) {
        descriptorWrites.push_back(sharedData.assetDescriptorSetLayout.getWriteDescriptorSet<3>(assetDescriptorSet, assetDescriptorSetReallocated ? 0 : 1, samplerInfos));
    }
    ...
    if (!inner.assetExtended->asset.textures.empty()) {
        ...
        for (const auto &chunk : usedImageIndices | std::views::chunk_by([](auto a, auto b) { return b - a == 1; })) {
            ...
            descriptorWrites.push_back(sharedData.assetDescriptorSetLayout.getWriteDescriptorSet<4>(assetDescriptorSet, 1 + chunk.front(), infos));
        }
    }
    ...
    if (std::uint32_t infoCount = inner.assetExtended->asset.textures.size() + assetDescriptorSetReallocated; infoCount > 0) {
        ...
        sharedData.gpu.device.updateDescriptorSets(
            sharedData.assetDescriptorSetLayout.getWriteDescriptorSet<3>(assetDescriptorSet, dstArrayElement, textureInfos),
            {});
    }
#endif
}
...
vk::raii::DescriptorPool vk_gltf_viewer::vulkan::Frame::createDescriptorPool() const {
    vku::DescriptorPoolSize poolSize;
    poolSize.add(sharedData.rendererDescriptorSetLayout);
    poolSize.add(sharedData.mousePickingDescriptorSetLayout);
    poolSize.add(sharedData.jumpFloodComputePipeline.descriptorSetLayout, 2);
    poolSize.add(sharedData.outlineDescriptorSetLayout, 2);
    poolSize.add(sharedData.weightedBlendedCompositionDescriptorSetLayout);
    poolSize.add(sharedData.inverseToneMappingDescriptorSetLayout);
    poolSize.add(sharedData.bloomComputePipeline.descriptorSetLayout);
    poolSize.add(sharedData.bloomApplyDescriptorSetLayout);
    poolSize.add(sharedData.assetDescriptorSetLayout);
