...
vk_gltf_viewer::vulkan::Frame::UpdateResult vk_gltf_viewer::vulkan::Frame::update(const ExecutionTask &task) {
    ...
    if (task.gltf) {
        ...
        if (global::frustumCullingMode != global::FrustumCullingMode::Off) {
            // std::ranges::for_each(renderingNodes->indirectDrawCommandBuffers | std::views::values, std::bind_back(commandBufferCullingFunc, global::camera.getFrustum()));
        ...
        if (task.gltf->selectedNodes) {
            ...
            if (global::frustumCullingMode != global::FrustumCullingMode::Off) {
                // std::ranges::for_each(selectedNodes->jumpFloodSeedIndirectDrawCommandBuffers | std::views::values, std::bind_back(commandBufferCullingFunc, global::camera.getFrustum()));
            }
            else {
        ...
        if (task.gltf->hoveringNode &&
            ...
            if (global::frustumCullingMode != global::FrustumCullingMode::Off) {
                // std::ranges::for_each(hoveringNode->jumpFloodSeedIndirectDrawCommandBuffers | std::views::values, std::bind_back(commandBufferCullingFunc, global::camera.getFrustum()));
            }
            else {
