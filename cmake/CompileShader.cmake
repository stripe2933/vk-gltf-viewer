    ...
    foreach (source IN LISTS ARGN)
        ...
        foreach (macro_value IN LISTS MACRO_VALUES)
            ...
            if (${Vulkan_glslc_FOUND})
                ...
                add_custom_command(
                    OUTPUT "${output}"
                    COMMAND ${Vulkan_GLSLC_EXECUTABLE} -MD -MF "${depfile}" $<$<NOT:$<CONFIG:Debug>>:-O> --target-env=vulkan1.2 -D${MACRO_NAME}=${macro_value} "${source}" -o "${output}"
                    DEPENDS "${source}"
                    BYPRODUCTS "${depfile}"
                    COMMENT "Compiling SPIRV: ${source} -> ${output}"
                    DEPFILE "${depfile}"
            ...
            elseif (${Vulkan_glslangValidator_FOUND})
                add_custom_command(
                    OUTPUT "${output}"
                    COMMAND ${Vulkan_GLSLANG_VALIDATOR_EXECUTABLE} -V --target-env vulkan1.2 "${source}" -D${MACRO_NAME}=${macro_value} -o "${output}"
                    DEPENDS "${source}"
                    COMMENT "Compiling SPIRV: ${source} -> ${output}"
                    VERBATIM
                    COMMAND_EXPAND_LISTS
