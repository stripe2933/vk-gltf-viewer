...
namespace ImGui {
    ...
    export ImVec2 CalcTextSize(std::string_view text, bool hide_text_after_double_hash = false, float wrapWidth = -1) {
        return CalcTextSize(text.data(), text.data() + text.size(), hide_text_after_double_hash, wrapWidth);
    }
    ...
    export void windowWithCenteredText(const char *windowName, std::string_view text) {
        if (Begin(windowName)) {
            SetCursorPos(ImVec2 { 0.5f, 0.5f } * (GetContentRegionAvail() - CalcTextSize(text)) + GetStyle().FramePadding);
