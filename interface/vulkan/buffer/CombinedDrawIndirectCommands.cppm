    ...
    public:
        struct Segment {
            ...
            [[nodiscard]] std::variant<std::span<const vk::DrawIndirectCommand>, std::span<const vk::DrawIndexedIndirectCommand>> getDrawIndirectCommands() const noexcept {
                if (indexed) {
                    return buffer->asRange<const vk::DrawIndexedIndirectCommand>(offset);
                }
                else {
                    return buffer->asRange<const vk::DrawIndirectCommand>(offset).subspan(0, drawCount);
                }
            }
            ...
            [[nodiscard]] std::variant<std::span<vk::DrawIndirectCommand>, std::span<vk::DrawIndexedIndirectCommand>> getDrawIndirectCommands() noexcept {
                if (indexed) {
                    return buffer->asRange<vk::DrawIndexedIndirectCommand>(offset);
                }
                else {
                    return buffer->asRange<vk::DrawIndirectCommand>(offset).subspan(0, drawCount);
                }
            }
            ...
            for (auto &[key, indexedAndDrawCommandBytes] : std::move(groupedDrawCommandBytes)) {
                ...
                if (indexed) {
                    result.try_emplace(
                        std::move(key),
                        this,
                        indexedDrawCommandBytesOffset + std::distance(indexedDrawCommandBytesStart, indexedDrawCommandBytesIt),
                        true,
