...
#ifdef EXACT_BOUNDING_VOLUME_USING_CGAL
                scenePoints.emplace_back(worldTransform.col(3).x(), worldTransform.col(3).y(), worldTransform.col(3).z());
#else
                min = cwiseMin(min, fastgltf::math::fvec3 { worldTransform.col(3) });
                max = cwiseMax(max, fastgltf::math::fvec3 { worldTransform.col(3) });
#endif
            }
        });
