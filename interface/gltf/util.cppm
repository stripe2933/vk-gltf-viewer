...
bool vk_gltf_viewer::gltf::isMaterialCompatible(const fastgltf::Material &material, const fastgltf::Primitive &primitive) noexcept {
	...
	for (const auto &[attributeName, _] : primitive.attributes) {
		...
		else if (constexpr std::string_view prefix = "TEXCOORD_"sv; attributeName.starts_with(prefix)) {
			int texcoordIndex;
			if (std::from_chars(&attributeName[prefix.size()], attributeName.data() + attributeName.size(), texcoordIndex).ec == std::errc()) {
				maxTexcoordIndex = std::max(maxTexcoordIndex, texcoordIndex);
			}
		}
	}

	if (const auto &texture = material.pbrData.baseColorTexture) {
		if (getTexcoordIndex(*texture) > maxTexcoordIndex) {
			return false;
		}
	}
	...
	if (const auto &texture = material.normalTexture) {
		if (!canHaveNormalTexture || getTexcoordIndex(*texture) > maxTexcoordIndex) {
			return false;
		}
	}
